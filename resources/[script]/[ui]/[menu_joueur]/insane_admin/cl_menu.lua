...
RMenu.Add('menu', 'main', RageUI.CreateMenu("CITIZEN-VICE", " "))
        ...
        Citizen.CreateThread(function()
			while VM.Staff do
				RageUI.IsVisible(RMenu:Get('menu', 'main'), true, true, true, function()
					...
					if InStaff then
						RageUI.Separator("↓ ~r~CITIZEN-VICE ~s~↓")
				...
				RageUI.IsVisible(RMenu:Get('menu', 'perso'), true, true, true, function()
                    ...
                    RageUI.Button("TP ADMIN ROOM",description, {}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                           ExecuteCommand('tpc 405.47 -961.22 -99.0')
                            end
                        end)
				...
				RageUI.IsVisible(RMenu:Get('menu', 'actionstaff'), true, true, true, function()
                   ...
                   RageUI.Button("Fix HUD", nil, {RightLabel = nil}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                        ExecuteCommand("ensure core_hud")
                        end
                    end)
                    ...
                    RageUI.Button("~g~BanList", nil, {RightLabel = nil}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                     ...
                     RageUI.Button("~o~Warn ", nil, {RightLabel = nil}, true, function(Hovered, Active, Selected)
                        if (Selected) then
                            ...
                            ExecuteCommand('bwh warn')
                                ...
                                RageUI.CloseAll()
                                          ...
                                          RageUI.Button("~g~Warn Liste ", nil, {RightLabel = nil}, true, function(Hovered, Active, Selected)
                        if (Selected) then
					...
					if Selected then
						if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
							...
							ESX.ShowNotification("~r~Erreur\n~s~Vous n'êtes pas dans un véhicule !")
                                        ...
                                        RageUI.Button("Supprimer Véhicule", nil, {}, true, function(_, Active, Selected)
                    if Selected then

                            local plaqueVehicule2 = Keyboardput("Plaque", "", 8)
                            TriggerServerEvent("AdvancedParking:deleteVehicle", plaqueVehicule2, true)
                            TriggerServerEvent("AdvancedParking:deleteVehicle", GetVehicleNumberPlateText(plaqueVehicule2), true)

                    end
                    end)

                  RageUI.Button("Faire le plein", nil, {}, true, function(_, Active, Selected)
                    if Selected then
                        local plyVeh = GetVehiclePedIsIn(PlayerPedId(), false)

                          exports["LegacyFuel"]:SetFuel(plyVeh, fuel_level)
             ...
             TriggerEvent("AdvancedParking:updateVehicle", vehicle)
              ...
              TriggerEvent("AdvancedParking:updateVehicle", vehicle)
				...
				RageUI.IsVisible(RMenu:Get('menu', 'time'), true, true, true, function()
					...
					RageUI.Button("Ouvrir menu Temps", "->", {RightLabel = "→"}, true, function(Hovered, Active, Selected)
						if (Selected) then
