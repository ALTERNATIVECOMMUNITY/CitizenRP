        ...
        if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
            ...
            elseif not Last and which == "Hat" then
                TriggerEvent('skinchanger:getSkin', function(skin)
                    PlayToggleEmote(Prop.Emote.On, function()
                        SetPedPropIndex(Ped, 0, skin.helmet_1, skin.helmet_2, true)
                    end)
                    LastEquipped[which] = false
                    return true
                end)
            elseif not Last and which == "Glasses" then
                TriggerEvent('skinchanger:getSkin', function(skin)
                    PlayToggleEmote(Prop.Emote.On, function()
                        SetPedPropIndex(Ped, 1, skin.glasses_1, skin.glasses_2, true)
                    end)
                    LastEquipped[which] = false
                    return true
                end)
...
function DrawDev() -- Draw text for all the stuff we are wearing, to make grabbing the variants of stuff much simpler for people.
    local Entries = {}
    for k, v in PairsKeys(Drawables) do
        table.insert(Entries, {
            Name = k,
            Drawable = v.Drawable
        })
    end
    for k, v in PairsKeys(Extras) do
        table.insert(Entries, {
            Name = k,
            Drawable = v.Drawable
        })
    end
    for k, v in PairsKeys(Props) do
        table.insert(Entries, {
            Name = k,
            Prop = v.Prop
        })
    ...
    for k, v in pairs(Entries) do
        ...
        if v.Drawable then
            Cur = {
                Id = GetPedDrawableVariation(Ped, v.Drawable),
                Texture = GetPedTextureVariation(Ped, v.Drawable)
            }
        elseif v.Prop then
            Cur = {
                Id = GetPedPropIndex(Ped, v.Prop),
                Texture = GetPedPropTextureIndex(Ped, v.Prop)
            }
        end
