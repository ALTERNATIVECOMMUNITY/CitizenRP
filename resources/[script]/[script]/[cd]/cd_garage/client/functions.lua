...
function LoadModel(model)
    if not HasModelLoaded(model) and IsModelInCdimage(model) then
        ...
        while not HasModelLoaded(model) do
            ...
            Draw2DText(Locales[Config.Language]['loading_model']..' : '..GetVehicleLabel_model(model)..' '..dots)
        end
    end
end
...
function GetVehicleLabel(vehicle)
    ...
    if vehicleLabel == 'null' or vehicleLabel == 'carnotfound' or vehicleLabel == 'NULL' then
        vehicleLabel = Locales[Config.Language]['vehicle']
    end
    if vehicleLabel ~= 'null' or vehicleLabel ~= 'carnotfound' or vehicleLabel ~= 'NULL'then
    ...
    for g = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
            if tostring(GetVehicleNumberPlateText(vehicle)) == tostring(plate) then
                ...
                if shell_coords ~= nil and GetVehicleEngineHealth(vehicle) > 0 then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsPedInAnyVehicle(ped, true) then
                ...
                Draw2DText(0.5, 0.05, '⬅️ '..Locales[Config.Language]['pre_livery']..'. ➡️ '..Locales[Config.Language]['next_livery']..'.\n ~b~['..Locales[Config.Language]['enter']..']~w~ '..Locales[Config.Language]['confirm']..'.')
...
function ChangeLivery(action, vehicle)
...
function OpenTextBox()
    local result = nil
    AddTextEntry('FMMC_KEY_TIP8s', 'Enter the plate (8 characters including spaces & CAPS ONLY)')
...
function SpawnVehicle(data, jobspawn, jobspawn_owned)
    InGarage = false
    ...
    local adv_stats
    if Config.UseMileage and not jobspawn then
    ...
    if HasModelLoaded(model) then
      ...
      SetPedIntoVehicle(ped, vehicle, -1)
        ...
        NetworkFadeInEntity(vehicle, true, true)
        exports['t1ger_keys']:GiveJobKeys(plate, 'Voiture de fonction', true)
        ...
        elseif Config.ChooseLiverys then
            if data.spawn_max then
                SetVehicleMaxMods(vehicle)
            end
            SetLiverysThread()
        end
        if jobspawn_owned and Config.ChooseLiverys then
            SetLiverysThread()
        end
        ExitLocation = nil
    else
                            ...
                            if MyCars[g].vehicle == vehicle.vehicle then
                                ...
                                if IsControlJustReleased(0, Config.QuickChoose_Key) then
                                    if not Config.Impound or GarageInfo[g].impound == 0 then
                                        if CheckProperty_inside(g) then
                                            if not Config.UniqueGarages or CurrentGarage == GarageInfo[g].garage_id or InProperty == true then
                                                ...
                                                elseif streetscheck.result == 'canspawn' then
                                                    SpawnVehicle(GarageInfo[g], false, false, false)
                                                end
                                            else
...
function CheckBlips(EnableBlip, JobRestricted)
    ...
    elseif JobRestricted ~= nil then
        local myjob = GetJob()
        for _, j in ipairs(JobRestricted) do
            if myjob == j then
                return true
            end
        end
...
RegisterNetEvent('cd_garage:VehicleManagement')
AddEventHandler('cd_garage:VehicleManagement', function(plate)
...
AddEventHandler('cd_garage:UpdateGarageType', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        TriggerServerEvent('cd_garage:UpdateGarageType', GetGarageType(vehicle), tostring(GetVehicleNumberPlateText(vehicle)))
    else
...
AddEventHandler('cd_garage:SetJobOwnedVehicle', function(action)
    if action == 'personal' or action == 'society' then
        ...
        local vehicle = GetVehiclePedIsIn(ped, false)
        if IsPedInAnyVehicle(ped, false) then
            TriggerServerEvent('cd_garage:SetJobOwnedVehicle', action, tostring(GetVehicleNumberPlateText(vehicle)))
        else
            Notif(3, 'get_inside_veh')
        end
    else
