    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour accéder à la ~b~Bank")
            else
            	inRange = true
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] Pour accéder à la banque', 'darkblue', 'left')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
        	if not Config.okokTextUI then
            	ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour accéder au ~b~ATM")
            else
            	inRange = true
            end
            ...
            if IsControlJustReleased(0, 38) then
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						exports['okokNotify']:Alert("BANK", "Rendez-vous dans une banque pour définir un code PIN", 5000, 'info')
					end
				end)
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] Pour accéder au guichet automatique', 'darkblue', 'left')
        elseif not inRange and shown then
        	shown = false
        	exports['okokTextUI']:Close()
        end
