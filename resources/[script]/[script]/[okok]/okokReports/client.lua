...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['okokNotify']:Alert("CITIZEN-V", "Vous ne pouvez pas signaler si rapidement !", 10000, 'warning')
	end
end, false)
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					exports['okokNotify']:Alert("REPORT", "Ce joueur n'est plus sur le serveur!", 20000, 'error')
				end
			end
		end
	elseif data.action == "concludeFeedback" then
		...
		if feedbackInfo then
			if feedbackInfo.concluded ~= true or canConclude then
				...
				exports['okokNotify']:Alert("FEEDBACK", "Feedback #"..feedbackID.." conclu!", 20000, 'success')
			end
		end
	end
end)
