...
AddEventHandler('esx:onPlayerDeath', function(data)
    ...
    --TriggerEvent("visn_phone:playerDied") -> Release soon... :)
		Citizen.Wait(5000)
    RespawnPed(ped, currentCoords, 0)
    ...
    setUnconscious(true, math.random(0, 20))
end)
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        ...
        if name == "CEventNetworkEntityDamage" then
            ...
            if entity == PlayerPedId() then
                ...
                print(damageType, bone, fatalDamage)
                ...
                if bone ~= nil and bone ~= 0 then
                    ...
                    if damageType == 487013001 and has_value(ConfigData.bleedReasons, "weapon_damage") then
                        shouldBleed = true
                    end

                    if damageType == 1593441988 and has_value(ConfigData.bleedReasons, "weapon_damage") then
                        shouldBleed = true
                    end

                    if damageType == -1660422300 and has_value(ConfigData.bleedReasons, "weapon_damage") then
                        shouldBleed = true
                    end

                    if damageType == -1074790547 and has_value(ConfigData.bleedReasons, "weapon_damage") then
                        shouldBleed = true
                    end

                    if damageType == -2084633992 and has_value(ConfigData.bleedReasons, "weapon_damage") then
                        shouldBleed = true
                    end
                    ...
                    if damageType == -1660422300 and not has_value(ConfigData.activatedReasons, "weapon_damage") then
                        return
                    end

                    if damageType == -1074790547 and not has_value(ConfigData.activatedReasons, "weapon_damage") then
                        return
                    end

                    if damageType == 487013001 and not has_value(ConfigData.activatedReasons, "weapon_damage") then
                        return
                    end
                    ...
                    if damageType == -1716189206 then
                        local damage = math.random(2, 3)
                        bloodAreas[pedBones[outBone][2]] = bloodAreas[pedBones[outBone][2]] + 2
                        damagedAreas[pedBones[outBone][2]] = damagedAreas[pedBones[outBone][2]] + 2
                    end
                    ...
                    if damageType == 539292904 and fatalDamage == 1 then
                        local head = math.random(1, 3)
                        local leftArm = math.random(2, 3)
                        local rightArm = math.random(2, 3)
                        local torso = math.random(1, 3)
                        local leftLeg = math.random(2, 3)
                        local rightLeg = math.random(2, 3)
...
AddEventHandler('visn_are:receiveEmergencyReviveKit', function(bodyPart)
	...
	RespawnPed(PlayerPedId(), GetEntityCoords(PlayerPedId(), 0), 0)
...
AddEventHandler('InteractSound_CL_M:PlayWithinDistance', function(playerCoords, maxDistance, soundFile, soundVolume)
...
function _useEmergencyReviveKit(playerId)
    ...
    ESX.TriggerServerCallback('visn_are:getItemCount', function(itemCount)
        if itemCount > 0 then
            createProgressbar(_translate('using_emr'), 120, 0.10)
...
function PutInBodybag()
    ...
    if isUnconscious and not attached then
        ...
        AttachEntityToEntity(bodyBag, playerPed, 0, -0.2, 0.75, -0.1, 0.0, 0.0, 0.0, false, false, false, false, 20, true)
        attached = true
        inBodybag = true
...
Citizen.CreateThread(function()
    while true do
        ...
        if isBusy then
            DisableControlAction(0, 104, true)-- ragdoll
            DisableControlAction(0, 24, true) -- Attack
            DisableControlAction(0, 91, true) -- Attack
            DisableControlAction(0, 92, true) -- Attack
            DisableControlAction(0, 69, true) -- Attack
            DisableControlAction(0, 68, true) -- Attack
            DisableControlAction(0, 70, true) -- Attack
            DisableControlAction(0, 330, true) -- Attack
            DisableControlAction(0, 331, true) -- Attack
            DisableControlAction(0, 257, true) -- Attack 2
            DisableControlAction(0, 25, true) -- Aim
            DisableControlAction(0, 26, true) -- Aim
        ...
        if not isUnconscious then
            ...
            if IsControlPressed(0, ConfigData.modifierKey) and IsControlJustPressed(0, ConfigData.mainKey) then
                ...
                if hit then
                    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(entity), true) <= 2.3 then
                        if IsEntityAPed(entity) and entity ~= PlayerPedId() then
                            EnableGui(true, GetPlayerServerId(NetworkGetPlayerIndexFromPed(entity)))
...
function setUnconscious(state, targetPulseParam)
    if not isUnconscious then
        isUnconscious = state
        ...
        if targetPulse == 0 then
            currentPulse = targetPulseParam
        else
...
Citizen.CreateThread(function()
    while true do
        ...
        if ConfigData.activateLimpWalkingStyle then
            if damagedAreas["left_leg"] > 2 or damagedAreas["right_leg"] > 2 then
...
Citizen.CreateThread(function()
	while true do
        ...
        Citizen.Wait(ConfigData.sewTimeout * 1000)
    end
end)
...
RegisterNetEvent('visn_are:ouverturemenu')
AddEventHandler('visn_are:ouverturemenu', function(state)
...
RegisterNUICallback('useEmergencyReviveKit', function(data)
    print(json.encode(data), data.personData.isMedic)
    if data.personData.isMedic and not data.personData.inBodybag then
        local victimPed = GetPlayerPed(GetPlayerFromServerId(data.source))
        local ownPed = PlayerPedId()
