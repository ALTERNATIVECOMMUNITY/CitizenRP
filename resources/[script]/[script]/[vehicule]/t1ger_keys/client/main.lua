...
-- Window Decors:
local window_decor = {[0] = "_WINDOW_LF", [1] = "_WINDOW_RF", [2] = "_WINDOW_LR", [3] = "_WINDOW_RR"}
for i = 0, #window_decor do DecorRegister(window_decor[i], true) end
-- Door Decors:
local door_decor = {[0] = "_DOOR_LF", [1] = "_DOOR_RF", [2] = "_DOOR_LR", [3] = "_DOOR_RR", [4] = "_DOOR_HOOD", [5] = "_DOOR_TRUNK"}
for i = 0, #door_decor do DecorRegister(door_decor[i], true) end
-- Engine Decor:
local engine_decor = "_ENGINE_RUNNING"
DecorRegister(engine_decor, false)
-- Neon Decor:
local neon_decor = {[0] = "_NEON_LIGHT_L", [1] = "_NEON_LIGHT_R", [2] = "_NEON_LIGHT_F", [3] = "_NEON_LIGHT_B"}
for i = 0, 3 do
	DecorRegister(neon_decor[i], false)
end
	...
	function(data, menu)
		...
		if data.current.value == 'veh_windows' then
			...
			function(data2, menu2)
				if not DecorExistOn(vehicle, window_decor[data2.current.index]) then
					DecorSetBool(vehicle, window_decor[data2.current.index], IsVehicleWindowIntact(vehicle, data2.current.index))
				end
				if DecorGetBool(vehicle, window_decor[data2.current.index]) then
					RollDownWindow(vehicle, data2.current.index)
					DecorSetBool(vehicle, window_decor[data2.current.index], false)
				else
					RollUpWindow(vehicle, data2.current.index)
					DecorSetBool(vehicle, window_decor[data2.current.index], true)
				end
		...
		if data.current.value == 'veh_doors' then
			...
			function(data2, menu2)
				if not DecorExistOn(vehicle, door_decor[data2.current.index]) then
					if GetVehicleDoorAngleRatio(vehicle, data2.current.index) > 0.0 then
						DecorSetBool(vehicle, door_decor[data2.current.index], true)
					else
						DecorSetBool(vehicle, door_decor[data2.current.index], false)
					end
				end
				if DecorGetBool(vehicle, door_decor[data2.current.index]) then
		...
		if data.current.value == 'veh_neon' then
			if DecorGetBool(vehicle, engine_decor) or GetIsVehicleEngineRunning(vehicle) then
				for i = 0, 3, 1 do
					if not DecorExistOn(vehicle, neon_decor[i]) then
						DecorSetBool(vehicle, neon_decor[i], IsVehicleNeonLightEnabled(vehicle, i))
					end
					if DecorGetBool(vehicle, neon_decor[i]) then
						DecorSetBool(vehicle, neon_decor[i], false)
					else
						DecorSetBool(vehicle, neon_decor[i], true)
					end
					SetVehicleNeonLightEnabled(vehicle, i, DecorGetBool(vehicle, neon_decor[i]))
				end
			else
...
function ToggleVehicleLock()
	...
	if DoesEntityExist(vehicle) then
		...
		if HasOwnedVehicleKey(plate) then
			SetVehicleLockedStatus(vehicle)
		else
			if HasAddedVehicleKey(plate) or HasJobVehicleKey(plate) then
				SetVehicleLockedStatus(vehicle)
			else
...
function SetVehicleLockedStatus(vehicle)
	-- animation:
	...
	if DecorGetInt(vehicle, lock_decor) == 1 or DecorGetInt(vehicle, lock_decor) == 0 then
		SetVehicleLocked(vehicle, Config.Lock.Status)
...
function HasAddedVehicleKey(plate)
					...
					if NPC_Vehicle ~= 0 then
						...
						if GetEntitySpeed(NPC_Vehicle) < Config.Steal.VehSpeed then
							...
							if Config.Steal.Locked then
								SetVehicleLocked(NPC_Vehicle, Config.Lock.Status)
							else
								SetVehicleLocked(NPC_Vehicle, 0)
							end
								...
								if not GetEntityPlayerIsFreeAimingAt(PlayerId(), entity) and tick > 0 then
									...
									if tick <= 0 then
										...
										if math.random(0,100) <= Config.Steal.Chance then
											...
											Citizen.Wait(1400)
											-- add keys to player:
											local plate = GetVehicleNumberPlateText(NPC_Vehicle)
...
function SetVehicleLocked(vehicle, int)
	if vehicle ~= 0 and DoesEntityExist(vehicle) then
		if tonumber(int) then
			DecorSetInt(vehicle, lock_decor, int)
		else
			return print("[SetVehicleLocked] status must be an int type")
		end
	else
...
function ToggleVehicleEngine(veh)
	local vehicle = veh
	T1GER_GetControlOfEntity(vehicle)
	if vehicle == nil or vehicle == 0 then
		...
		if DecorGetInt(vehicle, lock_decor) == 2 or DecorGetInt(vehicle, lock_decor) == 10 then
			return TriggerEvent('t1ger_keys:notify', Lang['veh_must_be_unlocked'])
		end
	end
	T1GER_GetControlOfEntity(vehicle)
