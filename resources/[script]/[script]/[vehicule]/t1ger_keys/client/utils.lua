...
-- Job Notification
RegisterNetEvent('t1ger_keys:job_notify')
AddEventHandler('t1ger_keys:job_notify', function(msg)
	if Config.DrugJobs.Settings.phoneMSG then
		local phone_number = Config.DrugJobs.NPC.name
		PlaySoundFrontend(-1, "Menu_Accept", "Phone_SoundSet_Default", true)
		TriggerEvent('t1ger_keys:notify', (Lang['new_msg_from']:format(phone_number)))
		TriggerServerEvent('gcPhone:sendMessage', phone_number, msg)
	else
		TriggerEvent('t1ger_keys:notify', msg)
	end
end)
