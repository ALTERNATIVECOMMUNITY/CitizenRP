...
ESX.RegisterServerCallback('t1ger_keys:fetchVehicleKey',function(source, cb, plate)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND plate = @plate', { ['@owner'] = xPlayer.identifier, ['@plate'] = plate }, function(result) 
		if result[1] then
...
AddEventHandler('t1ger_keys:giveCopyKeys',function(plate, name, target)
	...
	local veh_plate = string.gsub(plate, "^%s*(.-)%s*$", "%1")
...
function UpdateKeysToDatabase(plate, state)
	MySQL.Sync.execute('UPDATE owned_vehicles SET t1ger_keys = @t1ger_keys WHERE plate = @plate', { ['@plate'] = plate, ['@t1ger_keys'] = state}, function(added)
		if added then
...
AddEventHandler('t1ger_keys:registerAlarm',function(plate, state, price)
	...
	if money >= price then
		...
		MySQL.Async.execute("UPDATE owned_vehicles SET t1ger_alarm = @t1ger_alarm WHERE plate = @plate", {
			['@t1ger_alarm'] = state,
			['@plate'] = plate
		})
	else
