...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `plantWeed` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('plantWeed:planted',-1, id, location, stage)
    ...
    load = function(instance, callback)
        if onServer then
            verbose('Loading...')
            MySQL.Async.fetchAll("SELECT `id`, `stage`, UNIX_TIMESTAMP(`time`) AS `time`, `x`, `y`, `z`, `soil` FROM `plantWeed`;", 
            {},
            function(rows)
    ...
    import = function(instance, id, location, stage, time, soil)
        ...
        if not success then
            Citizen.Trace(string.format("plantWeed failed to import plant with ID %i into octree\n", id))
        end
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `plantWeed` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('plantWeed:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            MySQL.Async.execute("DELETE FROM `plantWeed` WHERE `id` = @id LIMIT 1;",
            { ['@id'] = id },
            function()
                TriggerClientEvent('plantWeed:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('plantWeed:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('plantWeed:bulk_data', target, forest)
        else
            TriggerServerEvent('plantWeed:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('plantWeed:request_data')
    AddEventHandler ('plantWeed:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('plantWeed:remove')
    AddEventHandler ('plantWeed:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('plantWeed:remove', src, plantID)
        end
    end)

    RegisterNetEvent('plantWeed:frob')
    AddEventHandler ('plantWeed:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('plantWeed:bulk_data')
    AddEventHandler ('plantWeed:bulk_data', function(forest)
    ...
    RegisterNetEvent('plantWeed:planted')
    AddEventHandler ('plantWeed:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('plantWeed:update')
    AddEventHandler ('plantWeed:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('plantWeed:removePlant')
    AddEventHandler ('plantWeed:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
