...
function makeToast(target, subject, message)
    TriggerClientEvent('plantWeed:make_toast', target, subject, message)
end
function inChat(target, message)
    if target == 0 then
        log(message)
    else
        TriggerClientEvent('chat:addMessage',target,{args={'plantWeed', message}})
    end
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('plantWeed:do', who, what, where)
end

RegisterNetEvent('plantWeed:success_plant')
AddEventHandler ('plantWeed:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('plantWeed:log')
AddEventHandler ('plantWeed:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('plantWeed:test_forest')
AddEventHandler ('plantWeed:test_forest',function(forest)
    local src = source


    if IsPlayerAceAllowed(src, 'command.plantWeed') then
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('plantWeed:attempt_plant', source)
        ...
        if state == "started" then
            ...
            cropstate:load(function(plantCount)
                if plantCount == 1 then
                    log('plantWeed loaded a single plant!')
                else
                    log('plantWeed loaded',plantCount,'plants')
                end
            end)
            databaseReady = true
        end
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantWeed:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantWeed:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantWeed:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantWeed:pyromaniac', source)
        end
    end,
}

RegisterCommand('plantWeed', function(source, args, raw)
    if #args > 0 then
