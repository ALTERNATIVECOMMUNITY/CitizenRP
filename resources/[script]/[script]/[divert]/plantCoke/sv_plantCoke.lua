...
function makeToast(target, subject, message)
    TriggerClientEvent('plantCoke:make_toast', target, subject, message)
end
function inChat(target, message)
    if target == 0 then
        log(message)
    else
        TriggerClientEvent('chat:addMessage',target,{args={'plantCoke', message}})
    end
end
...
function doScenario(who, what, where)
    ...
    TriggerClientEvent('plantCoke:do', who, what, where)
end

RegisterNetEvent('plantCoke:success_plant')
AddEventHandler ('plantCoke:success_plant', function(location, soil)
    local src = source
...
RegisterNetEvent('plantCoke:log')
AddEventHandler ('plantCoke:log',function(...)
    local src = source
    log(src,GetPlayerName(src),...)
end)

RegisterNetEvent('plantCoke:test_forest')
AddEventHandler ('plantCoke:test_forest',function(forest)
    local src = source


    if IsPlayerAceAllowed(src, 'command.plantCoke') then
                ...
                ESX.RegisterUsableItem(Config.Items.Seed, function(source)
                    ...
                    if now > last + (Config.ActionTime/1000) then
                        if HasItem(source, Config.Items.Seed) then
                            TriggerClientEvent('plantCoke:attempt_plant', source)
        ...
        if state == "started" then
            ...
            cropstate:load(function(plantCount)
                if plantCount == 1 then
                    log('plantCoke loaded a single plant!')
                else
                    log('plantCoke loaded',plantCount,'plants')
                end
            end)
            databaseReady = true
        end
...
local commands = {
    debug = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantCoke:toggle_debug', source)
        end
    end,
    ...
    forest = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantCoke:test_forest', source, count, randomStage)
    ...
    groundmat = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantCoke:groundmat', source)
        end
    end,
    pyro = function(source, args)
        if source == 0 then
            ...
            TriggerClientEvent('plantCoke:pyromaniac', source)
        end
    end,
}

RegisterCommand('plantCoke', function(source, args, raw)
    if #args > 0 then
