...
for i,entry in ipairs(registerStrings) do
    AddTextEntry('plantCoke_'..entry, _U(entry))
end

function interactHelp(stage,action)
    BeginTextCommandDisplayHelp('plantCoke_status_active')
...
function passiveHelp(stage,status)
    BeginTextCommandDisplayHelp('plantCoke_status_passive')
    -- AddTextComponentInteger(stage)
    -- AddTextComponentInteger(#Growth)
...
function serverlog(...)
    TriggerServerEvent('plantCoke:log',...)
end

RegisterNetEvent('plantCoke:make_toast')
AddEventHandler ('plantCoke:make_toast', function(subject,message)
...
Citizen.CreateThread(function()
    local hash = GetHashKey("a_m_y_acult_02")
...
RegisterNetEvent('plantCoke:do')
AddEventHandler ('plantCoke:do', function(scenarioName, location)
...
RegisterNetEvent('plantCoke:attempt_plant')
AddEventHandler ('plantCoke:attempt_plant', function()
    local plantable, message, location, _, soil = getPlantingLocation()
    if plantable then
        TriggerServerEvent('plantCoke:success_plant', location, soil)
                ...
                if closestDistance <= Config.Distance.Interact then
                    ...
                    if now >= lastAction + Config.ActionTime then
                        if IsDisabledControlJustPressed(0, 44) then
                            ...
                            TriggerServerEvent('plantCoke:remove', closestPlant.id, myLocation)
                        else
                            if stage.interact then
                                interactHelp(closestPlant.stage, _U(stage.label))
                                if IsControlJustPressed(0, 38) then
                                    lastAction = now
                                    TriggerServerEvent('plantCoke:frob', closestPlant.id, myLocation)
                                end
                            else
...
RegisterNetEvent('plantCoke:toggle_debug')
AddEventHandler ('plantCoke:toggle_debug', function()
...
RegisterNetEvent('plantCoke:pyromaniac')
AddEventHandler ('plantCoke:pyromaniac',function(location)
...
RegisterNetEvent('plantCoke:groundmat')
AddEventHandler ('plantCoke:groundmat', function()
    local plantable, message, where, normal, material = getPlantingLocation(true)
...
RegisterNetEvent('plantCoke:test_forest')
AddEventHandler ('plantCoke:test_forest',function(count, randomStage)
    local origin = GetEntityCoords(PlayerPedId())
    
    TriggerEvent("chat:addMessage", {args={'plantCoke','Target forest size: '..count}})
    local column = math.ceil(math.sqrt(count))
    TriggerEvent("chat:addMessage", {args={'plantCoke','Column size: '..column}})
    ...
    TriggerEvent("chat:addMessage", {args={'plantCoke', 'Actual viable locations: '..#forest}})
    TriggerServerEvent('plantCoke:test_forest', forest)
end)
