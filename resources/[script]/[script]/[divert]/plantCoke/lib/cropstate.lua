...
local cropstateMethods = {
    plant = function(instance, location, soil, stage)
        if onServer then
            stage = stage or 1
            MySQL.Async.insert("INSERT INTO `plantCoke` (`x`, `y`, `z`, `soil`, `stage`) VALUES (@x, @y, @z, @soil, @stage);",
            {
                ['@x'] = location.x,
                ['@y'] = location.y,
                ['@z'] = location.z,
            ...
            function(id)
                ...
                TriggerClientEvent('plantCoke:planted',-1, id, location, stage)
    ...
    load = function(instance, callback)
        if onServer then
            verbose('Loading...')
            MySQL.Async.fetchAll("SELECT `id`, `stage`, UNIX_TIMESTAMP(`time`) AS `time`, `x`, `y`, `z`, `soil` FROM `plantCoke`;", 
            {},
            function(rows)
    ...
    import = function(instance, id, location, stage, time, soil)
        ...
        if not success then
            Citizen.Trace(string.format("plantCoke failed to import plant with ID %i into octree\n", id))
        end
    ...
    update = function(instance, id, stage)
        ...
        if onServer then
            ...
            MySQL.Async.execute("UPDATE `plantCoke` SET `stage` = @stage WHERE `id` = @id LIMIT 1;",
            {
                ['@id'] = id,
                ['@stage'] = stage,
            }, function(_)
                TriggerClientEvent('plantCoke:update', -1, id, stage)
    ...
    remove = function(instance, id, withPyro)
        ...
        if onServer then
            MySQL.Async.execute("DELETE FROM `plantCoke` WHERE `id` = @id LIMIT 1;",
            { ['@id'] = id },
            function()
                TriggerClientEvent('plantCoke:removePlant', -1, id)
                if withPyro then
                    TriggerClientEvent('plantCoke:pyromaniac', -1, location)
                end
    ...
    bulkData = function(instance, target)
        if onServer then
            ...
            TriggerClientEvent('plantCoke:bulk_data', target, forest)
        else
            TriggerServerEvent('plantCoke:request_data')
        end
    end,
}
...
if onServer then
    RegisterNetEvent('plantCoke:request_data')
    AddEventHandler ('plantCoke:request_data', function()
        cropstate:bulkData(source)
    end)
    
    RegisterNetEvent('plantCoke:remove')
    AddEventHandler ('plantCoke:remove', function(plantID, nearLocation)
        local src = source
        ...
        if plant then
            ...
            TriggerClientEvent('plantCoke:remove', src, plantID)
        end
    end)

    RegisterNetEvent('plantCoke:frob')
    AddEventHandler ('plantCoke:frob', function(plantID, nearLocation)
        local src = source
        local plant = cropstate.index[plantID]
        if plant then
            local plantLocation = plant.bounds.location
    ...
    RegisterNetEvent('plantCoke:bulk_data')
    AddEventHandler ('plantCoke:bulk_data', function(forest)
    ...
    RegisterNetEvent('plantCoke:planted')
    AddEventHandler ('plantCoke:planted', function(id, location, stage)
        cropstate:import(id, location, stage)
    end)
    
    RegisterNetEvent('plantCoke:update')
    AddEventHandler ('plantCoke:update', function(plantID, stage)
        cropstate:update(plantID, stage)
    end)

    RegisterNetEvent('plantCoke:removePlant')
    AddEventHandler ('plantCoke:removePlant', function(plantID)
        cropstate:remove(plantID)
    end)
end
