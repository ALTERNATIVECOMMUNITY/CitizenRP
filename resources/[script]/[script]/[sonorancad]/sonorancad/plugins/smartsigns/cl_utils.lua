...
CreateThread(function() 
    Config.LoadPlugin("smartsigns", function(pluginConfig)
        if pluginConfig.enabled then
            Citizen.CreateThread(function()
                while true do
                    ...
                    for k, v in pairs(pluginConfig.signs) do
                        local distance = #(v.sign[1] - pedCoords)
                        if distance < pluginConfig.main.accessPointDistance then
                            nearAccessPoint(k, v, ped, pedCoords)
                        end
                        if distance <= pluginConfig.main.loadInDistance and not v.loaded then
                            if pluginConfig.main.developerMode then
                                print("[Smart Signs] - Loaded Sign ID: "..k)
                            end
                            TriggerEvent('loadSign', k)
                        elseif distance > pluginConfig.main.loadInDistance and v.loaded then
                            if pluginConfig.main.developerMode then
            ...
            function nearAccessPoint(k, v, ped, pedCoords)
                Draw3DText(v.sign[1].x + pluginConfig.main.signOffset[1], v.sign[1].y + pluginConfig.main.signOffset[2], v.sign[1].z + pluginConfig.main.signOffset[3], pluginConfig.main.instructionalText, 4, 0.05, 0.05)
                if IsControlJustPressed(pluginConfig.main.adjustButton[1], pluginConfig.main.adjustButton[2]) then
                    if pluginConfig.main.developerMode then
                        print("[Smart Signs] - Access point control pressed")
                    end
                    if pluginConfig.main.animation.enabled then
                        if pluginConfig.main.developerMode then
                        ...
                        RequestAnimDict(pluginConfig.main.animation.dict)
                        if pluginConfig.main.developerMode then
                            print("[Smart Signs] - Requesting Animation Dictionary: "..pluginConfig.main.animation.dict)
                        end
                        while not HasAnimDictLoaded(pluginConfig.main.animation.dict) do 
                            if pluginConfig.main.developerMode then
                                print("[Smart Signs] - Loading Animation Dictionary: "..pluginConfig.main.animation.dict)
                            end
                            Wait(0) 
                        end
                        if pluginConfig.main.developerMode then
                            print("[Smart Signs] - Loaded Animation Dictionary: "..pluginConfig.main.animation.dict)
                        end
                        if pluginConfig.main.developerMode then
                            print("[Smart Signs] - Playing Animation Name: "..pluginConfig.main.animation.name)
                        end
                        TaskPlayAnim(ped, pluginConfig.main.animation.dict, pluginConfig.main.animation.name, 8.0, -8.0, -1, 0, 0.0, 0, 0, 0)
                    end
                    local inputs = {}
                    ...
                    ClearPedTasks(ped)
                    if pluginConfig.main.soundEffect.enabled then
                        PlaySoundFrontend(-1, pluginConfig.main.soundEffect.name, pluginConfig.main.soundEffect.dict)
                        if pluginConfig.main.developerMode then
                            print("[Smart Signs] - Playing sound: Name: "..pluginConfig.main.soundEffect.name.." - Dict: "..pluginConfig.main.soundEffect.dict)
                        end
                    end
                end
            end
            ...
            if pluginConfig.main.developerMode then
                RegisterCommand("newsign", function()
                    RequestModel(pluginConfig.main.signModelName)
                    while not HasModelLoaded(pluginConfig.main.signModelName) do Wait(0) end
                    local ped = PlayerPedId()
                    local coords = GetEntityCoords(ped)
                    local signProp = CreateObject(pluginConfig.main.signModelName, coords, true, false, false)
                    while not DoesEntityExist(signProp) do Wait(0) end
                    FreezeEntityPosition(signProp, true)
                    SetEntityCoords(signProp, coords.x + pluginConfig.main.signOffset[1], coords.y + pluginConfig.main.signOffset[2], coords.z + pluginConfig.main.signOffset[3], true, true, true, false)
                    local heading = GetEntityHeading(ped)
                    SetEntityHeading(signProp, ped)
                    SetModelAsNoLongerNeeded(pluginConfig.main.signModelName)
