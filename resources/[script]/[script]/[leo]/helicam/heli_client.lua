...
local toggle_spotlight = 74 -- control id to toggle the front spotlight Default: H
local toggle_lock_on = 22 -- control id to lock onto a vehicle with the camera. Default is INPUT_SPRINT (spacebar)

-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("b412")
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerInPolmav() then
			...
			if GetLastInputMethod(2) and IsControlJustPressed(2, toggle_spotlight)  and GetPedInVehicleSeat(heli, -1) == lPed then
