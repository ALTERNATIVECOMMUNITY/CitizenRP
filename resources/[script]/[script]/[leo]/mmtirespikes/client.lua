...
local Config = {}
Config.Model = `p_ld_stinger_s`
Config.NoPermissionText = "Vous n'êtes pas autorisé à le faire!"
Config.GrabSpike = "Appuyez sur ~INPUT_CONTEXT ~pour~g~ saisir ~w~ la herse"
Config.ControlToGrabSpike = 38
Config.RemoveSpikeAfterDestroyingTire = false
...
RegisterNetEvent("mmtirespike:AddSpike", function()
    if Access then
        ...
        _ShowNotification(Config.NoPermissionText)
    end
end)
...
function CreateSpike(id, data)
    NumTireSpikes = NumTireSpikes + 1
    TireSpikes[id] = data
    _RequestModel(Config.Model)
	local obj = CreateObject(Config.Model, data.coords.x, data.coords.y, data.coords.z, false, false, false)
		...
		Citizen.CreateThread(function()
			while NumTireSpikes > 0 do
                ...
                for k,v in pairs(TireSpikes) do
                    ...
                    if distance < 2 and Access then
                        if IsPedOnFoot(playerPed) then
                            letSleep = false
                            SetTextComponentFormat("STRING")
                            AddTextComponentString(Config.GrabSpike)
                            DisplayHelpTextFromStringLabel(0, 0, 1, -1)
                            if IsControlPressed(0, Config.ControlToGrabSpike) then
                ...
                if cSpike ~= nil then
                    if isInVeh then
                        ...
                        if destroyed_tire and Config.RemoveSpikeAfterDestroyingTire then
                            TriggerServerEvent('mmtirespikes:RemoveSpike', cSpikeIndex)
                            Citizen.Wait(0)
                        end
                    end
                end
...
function GetSpikeDataForSpawn(coords, heading)
	_RequestModel(Config.Model)
	local obj = CreateObject(Config.Model, coords.x, coords.y, coords.z, false, false, false)
	SetEntityAsMissionEntity(obj, true, true)
	SetModelAsNoLongerNeeded(Config.Model)
...
exports("SetAccess", function(state) --- set permission to allow putting up / folding the sign etc.
	Access = state
	if Access then
		MainThread()
	end
