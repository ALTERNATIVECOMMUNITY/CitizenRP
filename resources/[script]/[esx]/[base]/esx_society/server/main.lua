	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_withdrawn', ESX.Math.GroupDigits(amount)) })
			else
				--xPlayer.showNotification(_U('invalid_amount'))

				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'error', text = _U('invalid_amount') })
			end
		end)
	else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('have_deposited', ESX.Math.GroupDigits(amount)) })
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			MySQL.Async.execute('INSERT INTO society_moneywash (identifier, society, amount) VALUES (@identifier, @society, @amount)', {
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have', ESX.Math.GroupDigits(amount)) })
			end)
		else
		--	xPlayer.showNotification(_U('invalid_amount'))
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'error', text = _U('invalid_amount') })
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget, { type = 'inform', text = _U('you_have_been_hired', job) })
			elseif type == 'promote' then
				--xTarget.showNotification(_U('you_have_been_promoted'))
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget, { type = 'inform', text = _U('you_have_been_promoted') })
			elseif type == 'fire' then
				--xTarget.showNotification(_U('you_have_been_fired', xTarget.getJob().label))
				TriggerClientEvent('mythic_notify:client:SendAlert', xTarget, { type = 'inform', text = _U('you_have_been_fired', xTarget.getJob().label) })
			end
	...
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)) })
			end
