	...
	while true do
		...
		if DistanceChecker then
			...
			if distance > 150 then
				...
				exports['mythic_notify']:SendAlert('error', _U('outside_jail'))
			end
		end
	end
end)
					...
					ESX.TriggerServerCallback('esx_extendedjail:haveitem', function(cb)
						if cb == true then
							if CanEscape then
								...
								exports['mythic_notify']:SendAlert('inform', _U('trying_to_computer'))
								...
								exports['mythic_notify']:SendAlert('inform', _U('alarm_fire'))
								...
								if Config.Mhacking then
									...
									exports.rprogress:Start("Hacking Terminal", Config.TimeToHack)
								...
								exports['mythic_notify']:SendAlert('error', _U('already_started_error'))
							end
						else
							--ESX.ShowNotification(_U('no_correct_item'))
							exports['mythic_notify']:SendAlert('error', _U('no_correct_item'))
						end
					end)
				end
			end
...
function hacksystem(success)
	if success then
		...
		exports['mythic_notify']:SendAlert('error', _U('failed_hacking_error'))
...
function JailPlayer(data)
	...
	elseif data.type == 'nofirst' then
		...
		exports['mythic_notify']:SendAlert('error', _U('already_jailed'))
		DistanceChecker = true
	end
end
...
function PJailPlayer(data)
	if data.type == 'first' then
		...
		exports['mythic_notify']:SendAlert('inform', _U('pjailed', data.time))
	elseif data.type == 'nofirst' then
		Citizen.Wait(5000)
		--ESX.ShowNotification(_U('already_pjailed'))

		exports['mythic_notify']:SendAlert('inform', _U('already_pjailed'))
	end
end
