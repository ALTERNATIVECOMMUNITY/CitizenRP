					...
					if Config.MaxInService == -1 then
						...
						ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
							if canTakeService then
								...
								exports['mythic_notify']:SendAlert('error', _U('service_full') .. inServiceCount .. '/' .. maxInService)
							end
						end, 'mechanic')
					end
...
-- 		local elements = {
-- 			{label = 'Boiute à outils', value = 'tool_box'},
-- 			{label = 'Outils de Mécano', value = 'mechanic_tools'},
-- 			{label = '', value = 'caro_tool'}
-- 		}
--
-- 		ESX.UI.Menu.CloseAll()
--
...
-- 			menu.close()
--
-- 			if data.current.value == 'tool_box' then
-- 				exports.rprogress:Start("Crafting", 2500)
-- 				TriggerServerEvent('esx_mechanicjob:startHarvest')
-- 			elseif data.current.value == 'mechanic_tools' then
-- 				exports.rprogress:Start("Crafting", 2500)
...
-- 		exports['mythic_notify']:SendAlert('error', _U('not_experienced_enough'))
-- 	end
-- end
--
...
-- 		--		exports.rprogress:Start("Crafting", 2500)
--
-- 			--	if ESX.PlayerData.canCarryItem('tool_box', 1) then
-- 			    ESX.PlayerData.addInventoryItem   ('tool_box', 1)
-- 		--	else
-- 			--    XPlayer.showNotification('Tu as les poches pleines !.')
-- 		--	end
--
--
--
...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elseif data.current.value == 'scan' then
			TriggerServerEvent('advanced_vehicles:showStatusUI')
		...
		elseif data.current.value == 'hijack_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					exports.rprogress:Start("Unlocking Vehicle", 10000)
					Citizen.Wait(10000)
		...
		elseif data.current.value == 'fix_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					exports.rprogress:Start("Fixing Vehicle", 20000)
					Citizen.Wait(20000)
		...
		elseif data.current.value == 'del_vehicle' then
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				...
				if GetPedInVehicleSeat(vehicle, -1) == playerPed then
					...
					exports['mythic_notify']:SendAlert('error', _U('vehicle_impounded'))
		...
		elseif data.current.value == 'dep_vehicle' then
			...
			if isVehicleTow then
				...
				if CurrentlyTowedVehicle == nil then
					if targetVehicle ~= 0 then
						if not IsPedInAnyVehicle(playerPed, true) then
							if vehicle ~= targetVehicle then
								...
								exports.rprogress:Start("Attaching Vehicle", 5000)
								Citizen.Wait(5000)
					...
					exports.rprogress:Start("Removing Vehicle", 5000)
					Citizen.Wait(5000)
		...
		elseif data.current.value == 'object_spawner' then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions_spawn', {
				...
				elements = {
					...
					{label = _U('spikes'),  value = 'p_ld_stinger_s'}
...
AddEventHandler('esx_mechanicjob:onHijack', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports.rprogress:Start("Unlocking", 10000)
				Citizen.Wait(10000)
				if chance <= 66 then
					SetVehicleDoorsLocked(vehicle, 1)
					SetVehicleDoorsLockedForAllPlayers(vehicle, false)
...
AddEventHandler('esx_mechanicjob:onCarokit', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				exports.rprogress:Start("Fixing Engine", 10000)
				Citizen.Wait(10000)
				SetVehicleFixed(vehicle)
				SetVehicleDeformationFixed(vehicle)
				ClearPedTasksImmediately(playerPed)
...
AddEventHandler('esx_mechanicjob:onFixkit', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			TaskStartScenarioInPlace(playerPed, 'PROP_HUMAN_BUM_BIN', 0, true)
			Citizen.CreateThread(function()
				exports.rprogress:Start("Fixing Body", 20000)
				Citizen.Wait(20000)
				SetVehicleFixed(vehicle)
				SetVehicleDeformationFixed(vehicle)
				SetVehicleUndriveable(vehicle, false)
...
Citizen.CreateThread(function()
	...
	local blip2 = AddBlipForCoord(1180.5984, 2647.6624, 37.8150)
		...
		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
				...
				if IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), GetHashKey('flatbed')) then
					...
					exports['mythic_notify']:SendAlert('error', _U('must_in_flatbed'))
				end
			end
		end
