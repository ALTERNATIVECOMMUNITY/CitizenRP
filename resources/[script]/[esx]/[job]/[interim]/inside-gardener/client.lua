            ...
            if PlayerData.job ~= nil and PlayerData.job.grade_name == 'gardener' then
                if (GetDistanceBetweenCoords(coords, Base.Pos.x, Base.Pos.y, Base.Pos.z, true) < 8) then
                    ...
                    if (GetDistanceBetweenCoords(coords, Base.Pos.x, Base.Pos.y, Base.Pos.z, true) < 1.2) then
                        if not onDuty then
                            sleep = 5
                            DrawText3Ds(Base.Pos.x, Base.Pos.y, Base.Pos.z + 0.4, '~g~[E]~s~ - Mettez des vêtements de travail')
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous avez commencé à travailler !", timeout = 3000})
                                onDuty = true
                                addGarageBlip()
                                exports.pNotify:SendNotification({text = "<b&gt;Jardinier</br> Pour ouvrir le menu de travail, appuyez sur <b>[DEL]</b>", timeout = 5000})
                            end
                        elseif onDuty then
                            sleep = 5
                            DrawText3Ds(Base.Pos.x, Base.Pos.y, Base.Pos.z + 0.4, '~r~[E]~s~ - Mettez des vêtements de citoyen')
                            if IsControlJustPressed(0, Keys["E"]) then
                                exports.rprogress:Custom({
                                    Duration = 2500,
                                    Label = "Vous changez de vêtements...",
                                ...
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Tu as fini le travail!", timeout = 3000})
                                onDuty = false
                                removeGarageBlip()
                            end
                        end
        ...
        local coords = GetEntityCoords(ped)
            ...
            if PlayerData.job ~= nil and PlayerData.job.grade_name == 'gardener' then
                if onDuty then
                    ...
                    elseif inMenu then
                        sleep = 2
                        DrawText3Dss(coords.x, coords.y, coords.z + 1.0, '~g~[7]~s~ - Rechercher une course | ~r~[8]~s~ - Annuler la course')
                        if IsControlJustPressed(0, Keys["DEL"]) then
                            inMenu = false
                        elseif IsControlJustPressed(0, Keys["7"]) then
                            if Type == nil then
                                inMenu = false
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Recherche d&#39;une course...", timeout = 15000})
                                Citizen.Wait(15000)
                                Gardens = Randomize(Config.Gardens)
                                CreateWork(Gardens.StreetHouse)
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Position GPS définie ! Conduire jusque" ..Gardens.StreetHouse, timeout = 2000})
                                ...
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous avez déjà une course !", timeout = 2000})
                            end
                        elseif IsControlJustPressed(0, Keys["8"]) then
                            if Type then
                                CancelWork()
                                DeleteWaypoint()
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous avez annulé une réunion avec un client", timeout = 2000})
                            elseif not Type then
                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous n&#39;avez actuellement aucun rendez-vous", timeout = 2000})
                            end
                        end
                    end
                end
                    ...
                    if (GetDistanceBetweenCoords(coords, Garage.Pos.x, Garage.Pos.y, Garage.Pos.z, true) < 8) then
                        ...
                        if (GetDistanceBetweenCoords(coords, Garage.Pos.x, Garage.Pos.y, Garage.Pos.z, true) < 1.2) then
                            if IsPedInAnyVehicle(ped, false) then
                                sleep = 5
                                DrawText3Ds(Garage.Pos.x, Garage.Pos.y, Garage.Pos.z + 0.4, '~r~[E]~s~ - Véhicule de retour')
                                if IsControlJustReleased(0, Keys["E"]) then
                                    if hasCar then
                                        TriggerServerEvent('inside-gardener:returnVehicle', source)
                                        ReturnVehicle()
                                        exports.pNotify:SendNotification({text = '<b>Jardinier</b></br> Tu as reçu ' ..Config.DepositPrice.. '$ pour le retour du véhicule', timeout = 1500})
                                        hasCar = false
                                        Plate = nil
                                    else
                                        exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous n&#39;avez pas payé d&#39;acompte pour ce véhicule !", timeout = 2500})
                                    end
                                end
                                ...
                                if not inGarageMenu then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        if not inMenu then
                                            ...
                                            inGarageMenu = true
                                            exports.pNotify:SendNotification({text = '<b>Jardinier</b></br> Sélectionnez une place de parking', timeout = 2500})
                                        elseif inMenu then
                                            exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Fermez le menu de travail !", timeout = 2500})
                                        end
                                    end
                                elseif inGarageMenu then
                                    DrawText3DMenu(Garage.Pos.x - 0.8, Garage.Pos.y, Garage.Pos.z + 0.8, '~g~[7]~s~ -Place de parking #1\n~g~[8]~s~ -Place de parking #2\n~r~[E]~s~ - Fermer le menu du garage ')
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        inGarageMenu = false
                                    ...
                                    elseif IsControlJustReleased(0, Keys["7"]) then
                                        if not hasCar then
                                            ...
                                            if hasMoney then
                                                ...
                                                exports.pNotify:SendNotification({text = '<b>Jardinier</b></br> Tu as payé ' ..Config.DepositPrice.. '$ pour sortir le véhicule', timeout = 1500})
                                                hasCar = true
                                                ...
                                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Vous n'avez pas assez d'argent !", timeout = 2500})
                                                inGarageMenu = false
                                                FreezeEntityPosition(ped, false)
                                            end
                                            end)
                                        elseif hasCar then
                                            exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> D'abord, pose la voiture que tu as sortie", timeout = 2500})
                                        end
                    ...
                    if Plate == GetVehicleNumberPlateText(vehicle) then
                        ...
                        if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 2) then
                            if not hasOpenDoor then
                                sleep = 5
                                DrawText3Ds(trunkpos.x, trunkpos.y, trunkpos.z + 0.4, "~g~[G]~s~ - Portes ouvertes")
                                if IsControlJustReleased(0, Keys["G"]) then
                                    exports.rprogress:Custom({
                                        Duration = 1500,
                                        Label = "Tu ouvres les portes arrière",
                                        DisableControls = {
                                            Mouse = false,
                                            Player = true,
                                            Vehicle = true
                            ...
                            elseif hasOpenDoor then
                                if not hasBlower and not hasLawnMower and not hasTrimmer and not hasBackPack then
                                    ...
                                    DrawText3Ds(trunkpos.x, trunkpos.y, trunkpos.z + 0.5, "~g~[7]~s~ - Tondeuse | ~g~[8]~s~ - Tondeuse à gazon")
                                    DrawText3Ds(trunkpos.x, trunkpos.y, trunkpos.z + 0.3, "~r~[G]~s~ - Fermez les portes")
                                    if IsControlJustReleased(0, Keys["G"]) then
                                        exports.rprogress:Custom({
                                            Duration = 1500,
                                            Label = "Tu fermes les portes arrière",
                                            DisableControls = {
                                                Mouse = false,
                                                Player = true,
                                                Vehicle = true
                                ...
                                elseif hasLawnMower or hasBlower or hasBackPack or hasTrimmer then
                                    sleep = 5
                                    DrawText3Ds(trunkpos.x, trunkpos.y, trunkpos.z + 0.5, "~g~[E]~s~ - Mettez l&#39;outil dans le coffre")
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        exports.rprogress:Custom({
                                            Duration = 1500,
                                            Label = "Tu mets l&#39;outil dans le coffre...",
                                            DisableControls = {
                                                Mouse = false,
                                                Player = true,
                                                Vehicle = true
                    ...
                    if not IsPedInAnyVehicle(ped, false) then
                        if not wasTalked then
                            if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 2.5) then
                                ...
                                DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '~g~[7]~s~ - Oui, bien sûr | ~r~[8]~s~ - Non, merci')
                                if IsControlJustReleased(0, Keys["7"]) then
                                    ...
                                    exports.pNotify:SendNotification({text = "<b&gt;Jardinier</br> Arracher les mauvaises herbes de la cour", timeout = 5000})
                                    BlipsWorkingRH()
                                elseif IsControlJustReleased(0, Keys["8"]) then
                                    wasTalked = true
                                    appointed = false
                        ...
                        elseif wasTalked then
                            if not appointed then
                                if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 3.5) then
                                    DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 1.05, "Bon, je vais trouver un meilleur jardinier !")
                            ...
                            elseif appointed then
                                if not waitingDone then
                                    if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 2.5) then
                                        DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 1.05, "Super! quand tu auras fini, préviens-moi, je serai à la porte d&#39;entrée")
                                    end
                                ...
                                elseif waitingDone then
                                    ...
                                    elseif Paycheck then
                                        if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 1.5) then
                                            DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 0.95, "Maintenant, le jardin a fière allure, voici votre chèque de paie")
                                            ...
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                ...
                                                exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Tu as gagné " ..salary.. "$!", timeout = 2500})
                                ...
                                if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.2) then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        if done == #Config.RockfordHillsWork then
                                            ...
                                            exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Le jardin est propre, va chercher ta paye", timeout = 2500})
                                        end
                                    end
                                end
                            end
            ...
            elseif Type == "West Vinewood" then
                for i, v in ipairs(Config.WestVinewood) do
                    ...
                    if not IsPedInAnyVehicle(ped, false) then
                        if not wasTalked then
                            if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 2.5) then
                                DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 1.05, 'Bonjour, voulez-vous me planter des arbres pour ~g~' ..salary.. '$~s~?')
                                DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '~g~[7]~s~ - Bien sûr | ~r~[8]~s~ - Pas question')
                                if IsControlJustReleased(0, Keys["7"]) then
                                    wasTalked = true
                                    appointed = true
                                    CanWork = true
                                    FreezeEntityPosition(v.ped, false)
                                    TaskGoToCoordAnyMeans(v.ped, v.houseX, v.houseY, v.houseZ, 1.3)
                                    exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Planter des arbres dans l&#39;allée", timeout = 5000})
                                    BlipsWorkingWV()
                                elseif IsControlJustReleased(0, Keys["8"]) then
                                    wasTalked = true
                                    appointed = false
                        ...
                        elseif wasTalked then
                            ...
                            elseif appointed then
                                ...
                                elseif waitingDone then
                                    ...
                                    elseif Paycheck then
                                        if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 1.5) then
                                            DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 0.95, "Maintenant, attendez qu&#39;ils grandissent ! garde ton salaire")
                                            DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '~g~[E]~s~ - Prends l\'argent')
                                            if IsControlJustReleased(0, Keys["E"]) then
                                                TaskTurnPedToFaceEntity(v.ped, ped, 0.2)
                                                TriggerServerEvent('inside-gardener:Payout', salary, AmountPayout)
                                ...
                                if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 1.2) then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        if done == #Config.WestVinewoodWork then
                                            ...
                                            AmountPayout = AmountPayout + 1
                                            exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Tous les arbres plantés, obtenez votre salaire", timeout = 2500})
                                        end
                                    end
                                end
                            end
            ...
            elseif Type == "Vinewood Hills" then
                for i, v in ipairs(Config.VinewoodHills) do
                    ...
                    if not IsPedInAnyVehicle(ped, false) then
                        if not wasTalked then
                            if (GetDistanceBetweenCoords(coords, coordsNPC.x, coordsNPC.y, coordsNPC.z, true) < 2.5) then
                                DrawText3Ds(coordsNPC.x, coordsNPC.y, coordsNPC.z + 1.05, 'Bonjour Monsieur, je veux gagner ~g~' ..salary.. '$~s~?')
                                DrawText3Ds(coords.x, coords.y, coords.z + 1.0, '~g~[7]~s~ - Volontiers | ~r~[8]~s~ - Tomber')
                                if IsControlJustReleased(0, Keys["7"]) then
                                    wasTalked = true
                                    appointed = true
                                    CanWork = true
                                    FreezeEntityPosition(v.ped, false)
                                    TaskGoToCoordAnyMeans(v.ped, v.houseX, v.houseY, v.houseZ, 1.3)
                                    exports.pNotify:SendNotification({text = "<b>Jardinier</b></br> Tirez le souffleur de feuilles du tronc et soufflez les feuilles du jardin", timeout = 5000})
                                    BlipsWorkingVH()
                                elseif IsControlJustReleased(0, Keys["8"]) then
                                    wasTalked = true
                                    appointed = false
