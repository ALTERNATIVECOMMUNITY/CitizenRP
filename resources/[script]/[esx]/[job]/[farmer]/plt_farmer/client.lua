...
local menuArea = false
local menuAsk = false
local menuClothe = false
		...
		while workStage ~= 0 and not iptalEdiyorum do Citizen.Wait(1000)
			...
			if bicerDover ~= false then 
				...
				if playerToBiverdover > max then 
					singleNotify('error', PLT.U["canceled_harvester"] ,25000)
					iptalEt()
					break
			...
			if traktor ~= false then 
				...
				if playerToTraktor > max then 
					singleNotify('error', PLT.U["canceled_tractor"] ,25000)
					iptalEt()
					break
			...
			if romork ~= false then 
				...
				if playerToRomork > max then 
					singleNotify('error', PLT.U["canceled_romork"] ,25000)
					iptalEt()
					break
			...
			if tir ~= false then 
				...
				if playerToTir > max then 
					singleNotify('error', PLT.U["canceled_tir"] ,25000)
					iptalEt()
					break
			...
			if dorse ~= false then 
				...
				if playerToDorse > max then 
					singleNotify('error', PLT.U["canceled_dorce"] ,25000)
					iptalEt()
					break
	...
	Citizen.CreateThread(function()
		...
		if not iptalEdiyorum then 
			for k,v in pairs(PLT.Area[myArea]) do 
				if IsAreaClear(vector3(v.x, v.y, v.z), 7.0) then
					SpawnVehicle("tractor3", vector3(v.x, v.y, v.z),v.w, function(vehicle)
...
function spawnTraktor(canIdo)
	...
	for k,v in pairs(PLT.VehicleSpawn) do 
		if IsAreaClear(vector3(v.x, v.y, v.z), 7.0) then
			SpawnVehicle(PLT.TractorName, vector3(v.x, v.y, v.z),v.w, function(vehicle)
				SetVehicleFuelLevel(vehicle, 99.0)
	...
	if basarili1 then 
		for k,v in pairs(PLT.VehicleSpawn) do 
			if IsAreaClear(vector3(v.x, v.y, v.z), 7.0) then
				SpawnVehicle("baletrailer", vector3(v.x, v.y, v.z),v.w, function(vehicle)
...
function spawnTir()
	...
	for k,v in pairs(PLT.VehicleSpawn) do 
		if IsAreaClear(vector3(v.x, v.y, v.z), 7.0) then
			SpawnVehicle(PLT.TruckName, vector3(v.x, v.y, v.z),v.w, function(vehicle)
				SetVehicleFuelLevel(vehicle, 99.0)
	...
	for k,v in pairs(PLT.VehicleSpawn) do 
		if IsAreaClear(vector3(v.x, v.y, v.z), 7.0) then
			SpawnVehicle("trflat", vector3(v.x, v.y, v.z),v.w, function(vehicle)
				dorse = vehicle
				bekle = false
...
function iptalEt()
	...
	goanim = false
	TriggerServerEvent("plt_farmer:CancelWork")
	if PLT.UseEsxMenu then ESX.UI.Menu.CloseAll() else 	SelectAreaMenu:Close() AskMenu:Close() ClotheMenu:Close()  end
	 myArea = false balyaOrani = nil

	if DoesEntityExist(bicerDover) then SetEntityAsMissionEntity(bicerDover, false, true) DeleteVehicle(bicerDover) end bicerDover = false
	if DoesEntityExist(traktor)  then SetEntityAsMissionEntity(traktor, false, true) DeleteVehicle(traktor) end traktor = false
	if DoesEntityExist(romork)  then SetEntityAsMissionEntity(romork, false, true) DeleteVehicle(romork) end romork = false
	if DoesEntityExist(tir)  then SetEntityAsMissionEntity(tir, false, true) DeleteVehicle(tir) end tir = false
	if DoesEntityExist(dorse)  then SetEntityAsMissionEntity(dorse, false, true) DeleteVehicle(dorse) end dorse = false
...
function IsAreaClear(coords, maxDistance)
	local allVehicle = GetGamePool('CVehicle')
	for k,v in pairs(allVehicle) do
		if #(coords - GetEntityCoords(v)) <= maxDistance then 
			return false
		end
	end
	return true
...
function SpawnVehicle(vehicle, coords, heading, cb, networked)
	Citizen.CreateThread(function()
		...
		if not HasModelLoaded(model) and IsModelInCdimage(model) then RequestModel(model) while not HasModelLoaded(model) do Citizen.Wait(4) end end
		...
		RequestCollisionAtCoord(coords.x,coords.y,coords.z) while not HasCollisionLoadedAroundEntity(vehicle) do Citizen.Wait(0) end
