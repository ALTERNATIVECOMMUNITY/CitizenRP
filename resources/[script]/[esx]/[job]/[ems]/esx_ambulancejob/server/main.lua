		...
		if xTarget then
			if deadPlayers[playerId] then
				if Config.ReviveReward > 0 then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('revive_complete_award', xTarget.name, Config.ReviveReward) })
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('revive_complete', xTarget.name) })
					xTarget.triggerEvent('esx_ambulancejob:revive')
					deadPlayers[playerId] = nil
				end
			else
				--xPlayer.showNotification(_U('player_not_unconscious'))
				TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'error', text = _U('player_not_unconscious') })
			end
		else
			--xPlayer.showNotification(_U('revive_fail_offline'))

			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'error', text = _U('revive_fail_offline') })
		end
	end
end)
...
if Config.EarlyRespawnFine then
	...
	AddEventHandler('esx_ambulancejob:payFine', function()
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('respawn_bleedout_fine_msg', ESX.Math.GroupDigits(fineAmount)) })
...
AddEventHandler('esx_ambulancejob:removeItem', function(item)
	...
	elseif item == 'medikit' then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'inform', text = _U('used_medikit') })
	end
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	if xPlayer.canCarryItem(itemName, amount) then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', xPlayer, { type = 'error', text = _U('max_item') })
	end
end)

ESX.RegisterCommand('revive', 'admin', function(xPlayer, args, showError)
    args.playerId.triggerEvent('esx_ambulancejob:revive')
    args.playerId.triggerEvent('visn_are:resetValues')
