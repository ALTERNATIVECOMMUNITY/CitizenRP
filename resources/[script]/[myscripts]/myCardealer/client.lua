...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('myCardealer:getVehicles')
    GetJob()
end)
    ...
    while true do
        ...
        if (inRing) then
            ...
            if IsControlJustReleased(1, 38) then --38 = E Key
               TriggerServerEvent('myCardealer:RingRing')
            end
        end
...
---- Client Events ----
RegisterNetEvent('myCardealer:ShowMSG')
AddEventHandler('myCardealer:ShowMSG', function(msg, OnlyForJob)
...
RegisterNetEvent('myCardealer:ResultSocietyMoney')
AddEventHandler('myCardealer:ResultSocietyMoney', function(money)
    currentSocietyMoney = money
    gotDataMoney = true
end)

RegisterNetEvent('myCardealer:getEmployeesResult')
AddEventHandler('myCardealer:getEmployeesResult', function(employee)
    employees = {}
    employees = employee
    gotEmployees = true
end)
...
RegisterNetEvent('myCardealer:ShowAdvancedMSG')
AddEventHandler('myCardealer:ShowAdvancedMSG', function(pic, title, subtitle, msg)
    ShowAdvancedNotification(pic, title, subtitle, msg)
end)

RegisterNetEvent('myCardealer:getVehiclesResult')
AddEventHandler('myCardealer:getVehiclesResult', function(Data)
    Vehicles = {}
    Vehicles = Data
    GetJob()
end)

RegisterNetEvent('myCardealer:CheckTimeResult')
AddEventHandler('myCardealer:CheckTimeResult', function(data)
    timeToLiver = data
end)
---- functions ----
...
function GenerateBossMenu()
    ...
    TriggerServerEvent('myCardealer:getSocietyMoney')
    TriggerServerEvent('myCardealer:getEmployeesDB')
    ...
    currentMoney:RightLabel('~HUD_COLOUR_GREENDark~' .. currentSocietyMoney .. '$')
		...
		local onScreenDisplayResult_shopmenu_amount = CreateDialog(Translation[Cfg.Locale]['how_much'])
			if (onScreenDisplayResult_shopmenu_amount ~= nil) then
				if tonumber(onScreenDisplayResult_shopmenu_amount) then
                    ...
                    TriggerServerEvent('myCardealer:addSocietyMoney', amount)
    ...
    for j, ply in pairs(players) do
        ...
        hirePlayerItem.Activated = function(sender, index)
            TriggerServerEvent('myCardealer:hire', GetPlayerServerId(ply), GetPlayerName(ply))
        end
    end
    ...
    for i, emp in pairs(employees) do
        ...
        for o, grade in pairs(Cfg.Grades) do
            ...
            grade_item.Activated = function(sender, item)
                TriggerServerEvent('myCardealer:promote', emp.identifier, emp.firstname, emp.lastname, grade.number, grade.label)
            end
        end
        fire.Activated = function(sender, item)
            TriggerServerEvent('myCardealer:fire', emp.identifier, emp.firstname, emp.lastname)
            TriggerServerEvent('myCardealer:getEmployeesDB')
        end
    end
...
function GenerateComputerMenu(x, y, z, rot)
    ...
    if Distance ~= -1 and Distance < 5.0 then
        ...
        local ServerIdtargetPlayer = GetPlayerServerId(player)
        ...
        sell_item.Activated = function(sender, item)
            ...
            local cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", Cfg.Coords.cam.pos.x, Cfg.Coords.cam.pos.y, Cfg.Coords.cam.pos.z, 300.00,0.00,0.00, 100.00, false, 0)
            local cam2 = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", Cfg.Coords.cam2.pos.x, Cfg.Coords.cam2.pos.y, Cfg.Coords.cam2.pos.z, 300.00,0.00,0.00, 100.00, false, 0)
            local cam3 = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", Cfg.Coords.cam3.pos.x, Cfg.Coords.cam3.pos.y, Cfg.Coords.cam3.pos.z, 300.00,0.00,0.00, 100.00, false, 0)
            local cam4 = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", Cfg.Coords.cam4.pos.x, Cfg.Coords.cam4.pos.y, Cfg.Coords.cam4.pos.z, 300.00,0.00,0.00, 100.00, false, 0)
            local currCam = cam
                            ...
                            if tonumber(price) then
                                ...
                                if vehProps ~= nil then
                                    if price ~= nil then
                                        ...
                                        -- MONEY Check
                                        TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_cardealer', Translation[Cfg.Locale]['bill_purchase_of'] .. v.manufacturer .. ' ' .. v.label, price)
                                        ...
                                        TriggerServerEvent('myCardealer:BuyVeh', src, v.manufacturer, v.label, vehProps, v.storage, GetDisplayNameFromVehicleModel(vehHash), price, NetworkGetNetworkIdFromEntity(veh), GetEntityCoords(veh), GetEntityHeading(veh))
                                        GiveCarBack(veh)
                                    end
                                else
                            ...
                            if tonumber(price) then
                                ...
                                if vehProps ~= nil then
                                    if price ~= nil then
                                        ...
                                        TriggerServerEvent('myCardealer:BuyVeh', src, v.manufacturer, v.label, vehProps, v.storage, GetDisplayNameFromVehicleModel(vehHash), price, NetworkGetNetworkIdFromEntity(veh2), GetEntityCoords(veh2), GetEntityHeading(veh2))
                                        GiveCarBack(veh)
                                    end
                                else
                                    ShowNotification('~r~An error occured, please notify the server owner! Error: vehProps = nil')
                            ...
                            if tonumber(price) then
                                ...
                                if vehProps ~= nil then
                                    if price ~= nil then
                                        ...
                                        TriggerServerEvent('myCardealer:BuyVeh', src, v.manufacturer, v.label, vehProps, v.storage, GetDisplayNameFromVehicleModel(vehHash), price, NetworkGetNetworkIdFromEntity(veh3), GetEntityCoords(veh3), GetEntityHeading(veh3))
                                        GiveCarBack(veh)
                                    end
                                else
                                    ShowNotification('~r~An error occured, please notify the server owner! Error: vehProps = nil')
                            ...
                            if tonumber(price) then
                                ...
                                if vehProps ~= nil then
                                    if price ~= nil then
                                        ...
                                        TriggerServerEvent('myCardealer:BuyVeh', src, v.manufacturer, v.label, vehProps, v.storage, GetDisplayNameFromVehicleModel(vehHash), price, NetworkGetNetworkIdFromEntity(veh4), GetEntityCoords(veh4), GetEntityHeading(veh4))
                                        GiveCarBack(veh)
                                    end
                                else
                                    ShowNotification('~r~An error occured, please notify the server owner! Error: vehProps = nil')
    ...
    for k,v in pairs(Vehicles) do
        ...
        ps_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('myCardealer:updateInfo', v.id, 'ps', newValue)
            end
        end
        ...
        maxSpeed_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('myCardealer:updateInfo', v.id, 'maxSpeed', newValue)
            end
        end
        ...
        speedup.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('myCardealer:updateInfo', v.id, 'speedUp', newValue)
            end
        end
        ...
        modifikation.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('myCardealer:updateInfo', v.id, 'modification', newValue)
            end
        end
        ...
        tankcapacity.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('myCardealer:updateInfo', v.id, 'fuelcapacity', newValue)
            end
        end
        ...
        if isBoss then
            ...
            liver.Activated = function(sender, item)
                ...
                if tonumber(ordertotal) then
                    ...
                    TriggerServerEvent('myCardealer:liverVehicle', v.model, ordertotal, v.label, v.manufacturer)
                end
            end
            ...
            produce.Activated = function(sender, item)
                ...
                if tonumber(producetotal) then
                    ...
                    TriggerServerEvent('myCardealer:produceVehicle', v.model, producetotal, v.label, v.manufacturer)
                end
            end
            if v.orderd == 1 then
        ...
        for k,v in pairs(Vehicles) do
            if v.produced == 1 then
                ...
                check.Activated = function(sender, item)
                    TriggerServerEvent('myCardealer:CheckTime', v.time, v.lastorder, v.model, v.storage, v.producetotal)
                end
            end
        end
    end
...
RegisterCommand("ClientCardealer", function(source, args, rawCommand)
    TriggerServerEvent('myCardealer:getVehicles')
end, false)
