...
function createBillsMenu()
	...
	if Config.CanEverybodyGiveBills then
		...
		local isSocietyBillCheckbox = NativeUI.CreateCheckboxItem(Translation[Config.Locale]['is_society_bill'], isSocietyBill, '~b~')
		giveBill_sub:AddItem(isSocietyBillCheckbox)

		giveBill_sub.OnCheckboxChange = function(sender, item, checked_)
			if item == isSocietyBillCheckbox then
				isSocietyBill = checked_
			end
		end
			...
			if amountInt ~= nil and amountInt > 0 then
				...
				if closestPlayer == -1 or closestDistance > 4.0 then
					...
					if isSocietyBill then
						TriggerServerEvent('esx_billing:sendSocietyBill',  GetPlayerServerId(closestPlayer), nil, reasonStr, amountInt)
					else
...
AddEventHandler('myBills:gotInvoice', function(senderPlayer, playerIdentifier, sharedAccountName, label, amount)
	...
	SendNUIMessage({money = amount, reason = label, title = Translation[Config.Locale]['invoice_title'], priceTitle = Translation[Config.Locale]['invoice_pricetitle'], reasonTitle = Translation[Config.Locale]['invoice_reasontitle'], signTitle = Translation[Config.Locale]['invoice_sign'], acceptTitle = Translation[Config.Locale]['invoice_accepttitle'], senderPlayer = senderPlayer, playerIdentifier = playerIdentifier, sharedAccountName = sharedAccountName})
...
RegisterNUICallback('accept', function(data)
	TriggerServerEvent('myBills:gotAnswerForBill', data.senderPlayer, data.playerIdentifier, data.sharedAccountName, data.label, data.amount, true)
	SetNuiFocus(false, false)
end)

RegisterNUICallback('denied', function(data)
	TriggerServerEvent('myBills:gotAnswerForBill', data.senderPlayer, data.playerIdentifier, data.sharedAccountName, data.label, data.amount, false)
	SetNuiFocus(false, false)
end)
