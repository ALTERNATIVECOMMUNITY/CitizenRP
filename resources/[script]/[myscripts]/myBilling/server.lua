...
AddEventHandler('esx_billing:sendSocietyBill', function(playerId, sharedAccountName, label, amount)
    ...
    TriggerEvent('esx_society:getSociety', xPlayer.job.name, function (society)
        --print('foundSociety: ' .. society.account)
        if society ~= nil then
            currentSociety = society.account
        else
            TriggerClientEvent('myBills:msg', source, Translation[Config.Locale]['no_society'])
            return
        end
    ...
    if amount > 0 then
        ...
        for k, v in pairs(xPlayers) do
            ...
            if xPlayer2.source == playerId then
                ...
                TriggerClientEvent('myBills:msg', source, Translation[Config.Locale]['bill_given'])
                --TriggerClientEvent('myBills:gotInvoice', playerId, source, xPlayer2.identifier, sharedAccountName, label, amount)
				TriggerClientEvent('myBills:gotInvoice', playerId, source, xPlayer2.identifier, currentSociety, label, amount)
                break
            end
...
RegisterServerEvent('esx_billing:sendBillDirect')
AddEventHandler('esx_billing:sendBillDirect', function(playerId, sharedAccountName, label, amount)
    ...
    if amount > 0 then
        ...
        for k, v in pairs(xPlayers) do
            ...
            local xPlayer2 = ESX.GetPlayerFromId(xPlayers[k])
            if xPlayer2.source == playerId then
				TriggerEvent('myBills:gotAnswerForBill', _source, xPlayer2.identifier, sharedAccountName, label, amount, true)
                break
            end
