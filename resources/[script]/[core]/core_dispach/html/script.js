...
$(document).keyup(function (e) {
  if (e.keyCode === 27) {
    ...
    $.post('https://core_dispach/close', JSON.stringify({}));
...
function forwardCall(num) {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == num) {
      $.post('https://core_dispach/forwardCall', JSON.stringify({
...
function copyNumber() {
  ...
  $.post('https://core_dispach/copynumber', JSON.stringify({}));
}

function toggleOffduty() {

  $.post('https://core_dispach/toggleoffduty', JSON.stringify({}));

}

function togglecallblips() {
  $.post('https://core_dispach/togglecallblips', JSON.stringify({}));
}

function toggleUnitBlips() {
  $.post('https://core_dispach/toggleunitblips', JSON.stringify({}));
}

function toggleAlerts() {

  $.post('https://core_dispach/togglealerts', JSON.stringify({}));

}

function changeToBusy() {
  $.post('https://core_dispach/updatestatus', JSON.stringify({
    id: id,
    status: 'Busy'
  }));
}
...
function dismissCall() {
  ...
  acceptedCall = '';
  $.post('https://core_dispach/dismissCall', JSON.stringify({
    id: currentElement.id
  }));
...
function removeCall() {
  ...
  $.post('https://core_dispach/removeCall', JSON.stringify({
    id: currentElement.id
  }));
...
function acceptCall() {
  ...
  $.post('https://core_dispach/acceptCall', JSON.stringify({
    id: currentElement.id,
  ...
  $(currentElement).find("#u4538-7").append('<div class="unit ' + jobInfo[job].color + '" id="unit_' + id + '">' + getCallSign(id) + '</div>');

  $.post('https://core_dispach/sendnotice', JSON.stringify({
...
function requestBackup() {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == col) {
      $.post('https://core_dispach/reqbackup', JSON.stringify({
        job: key,
