...
AddEventHandler('gl-pets:spawnPed',function(coords,heading)
    ...
    exports['bt-target']:AddBoxZone('petShopNPC',vector3(563.76,2753.27,41.88), 0.4, 0.6, {
            name="petShopNPC",
            debugPoly=false,
            useZ = true
                }, {
...
Citizen.CreateThread(function()
        blip2 = AddBlipForCoord(-1554.7,-968.91,12.02)
        ...
        BeginTextCommandSetBlipName("STRING")
        AddTextComponentString("Pet Vet")
        EndTextCommandSetBlipName(blip2)

    while true do
        Citizen.Wait(1000)
            local pedCoords = GetEntityCoords(PlayerPedId())
            local spawnCoords = vector3(-1554.7,-968.91,12.02)
            local dst = #(spawnCoords - pedCoords)

            if dst < 100 and vetSpawned == false then
                TriggerEvent('gl-pets:spawnVet',spawnCoords,226.83)
                vetSpawned = true
            end
            if dst >= 101  then
                vetSpawned = false
...
AddEventHandler('gl-pets:spawnVet',function(coords,heading)
    local hash = GetHashKey('mp_f_counterfeit_01')
    if not HasModelLoaded(hash) then
        RequestModel(hash)
        Wait(10)
    end
    ...
    SetModelAsNoLongerNeeded(hash)
                exports['bt-target']:AddBoxZone('vet',vector3(-1554.7,-968.91,12.02), 0.4, 0.6, {
                        name="vet",
                        debugPoly=false,
                        useZ = true
                              }, {
...
Citizen.CreateThread(function()
    while true do
        ...
        if DoesEntityExist(pet) then
            ...
            if dst > 5 and not petMoving and not stay and not feeding and not chasing and not attacking then
                petMoving = true
