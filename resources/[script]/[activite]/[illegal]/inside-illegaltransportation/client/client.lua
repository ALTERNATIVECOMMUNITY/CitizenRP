			...
			if(GetDistanceBetweenCoords(coords,Config.illegaltransportation['BossSpawn'].Pos.x, Config.illegaltransportation['BossSpawn'].Pos.y, Config.illegaltransportation['BossSpawn'].Pos.z, true) < 9.0) then
				...
				if(GetDistanceBetweenCoords(coords,Config.illegaltransportation['BossSpawn'].Pos.x, Config.illegaltransportation['BossSpawn'].Pos.y, Config.illegaltransportation['BossSpawn'].Pos.z, true) < 1.5) then
					if not JobStarted then
						DrawText3Ds(Config.illegaltransportation['BossSpawn'].Pos.x, Config.illegaltransportation['BossSpawn'].Pos.y, Config.illegaltransportation['BossSpawn'].Pos.z+1.0, 'Pour commencer à travailler, appuyez sur [~g~E~w~]')
						...
						DrawText3Ds(Config.illegaltransportation['BossSpawn'].Pos.x, Config.illegaltransportation['BossSpawn'].Pos.y, Config.illegaltransportation['BossSpawn'].Pos.z+1.0, 'Pour interrompre une affectation, appuyez sur[~r~E~w~]')
					...
					if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not JobStarted then
						JobStarted = true
						exports.pNotify:SendNotification({text = "<b>Boss</b></br>Prends ma voiture et va à ta première destination", timeout = 4500})
					...
					elseif IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and JobStarted then
						...
						anim = false
						minus = 0
						exports.pNotify:SendNotification({text = "<b>Boss</b></br>C'est dommage que tu ne puisses pas y arriver ... T'es vraiment une merde ...", timeout = 4500})
						for i, v in pairs(Config.Package) do
							DeleteEntity(v.pack)
						end
						for i, v in pairs(Config.Places[Place.name].Peds['Peds']) do
				...
				if(GetDistanceBetweenCoords(coords,Config.illegaltransportation['VehicleSpawn'].Pos.x, Config.illegaltransportation['VehicleSpawn'].Pos.y, Config.illegaltransportation['VehicleSpawn'].Pos.z, true) < 1.5) then
					if not Vehicleout then
						...
						if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(ped, false) and not Vehicleout then
							if ESX.Game.IsSpawnPointClear(vector3(Config.illegaltransportation['VehicleSpawn'].Pos.x, Config.illegaltransportation['VehicleSpawn'].Pos.y, Config.illegaltransportation['VehicleSpawn'].Pos.z), 2) then
								ESX.Game.SpawnVehicle(Config.illegaltransportation['VehicleSpawn'].Type, vector3(Config.illegaltransportation['VehicleSpawn'].Pos.x, Config.illegaltransportation['VehicleSpawn'].Pos.y, Config.illegaltransportation['VehicleSpawn'].Pos.z), Config.illegaltransportation['VehicleSpawn'].Pos.h, function(vehicle)
									Vehicle = vehicle
								...
								exports.pNotify:SendNotification({text = "<b>Transport illégal</b></br>Le site de spawn est bloqué !", timeout = 4000})
							end
						end
					else
		...
		local coords = GetEntityCoords(ped)
		 	if haspackage or haspackage2 then
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 2.5 then
					DrawText3Ds(trunkpos.x, trunkpos.y, trunkpos.z + 0.25, 'Pour poser la drogue, appuyez sur [~g~E~w~]')
	...
	while true do
		...
		if Vehicleout then
			...
			if IsPedDeadOrDying(ped, 1) then
				...
				Place = nil
				exports.pNotify:SendNotification({text = "<b>Illegal Transportation</b></br>La mission s'est terminée parce que tu es mort!", timeout = 4000})
			end
		end
		Citizen.Wait(sleep)
	end
		...
		if Vehicleout then
			...
			if not JobDone then
				...
				if(GetDistanceBetweenCoords(coords,Config.Places[Place.name].DrugPos.x, Config.Places[Place.name].DrugPos.y, Config.Places[Place.name].DrugPos.z, true) < 1.5) and DrugObject and not (haspackage or haspackage2) then
					...
					DrawText3Ds(Config.Places[Place.name].DrugPos.x, Config.Places[Place.name].DrugPos.y, Config.Places[Place.name].DrugPos.z+1.0, 'Pour prendre de la drogue, appuyer [~g~E~w~]')
					if DrugTaken < 2 then
				...
				elseif(GetDistanceBetweenCoords(coords,Config.Places[Place.name].DrugPos2.x, Config.Places[Place.name].DrugPos2.y, Config.Places[Place.name].DrugPos2.z, true) < 1.5) and DrugObject2 and (not haspackage2 or haspackage) then
					...
					DrawText3Ds(Config.Places[Place.name].DrugPos2.x, Config.Places[Place.name].DrugPos2.y, Config.Places[Place.name].DrugPos2.z+1.0, 'Pour prendre de la drogue, appuyez sur [~g~E~w~]')
				...
				elseif not DrugObject2 and not DrugObject then
					if packagetrunk >= 11 then
						exports.pNotify:SendNotification({text = "<b>Transport illégal</b></br>Retournez au payeur pour le paiement!", timeout = 4000})
						PlaceDepotPayoutB()
						while true do
							local sleep = 500
							local ped = PlayerPedId()
							...
							if(GetDistanceBetweenCoords(coords,Config.illegaltransportation['VehicleDepot'].Pos.x, Config.illegaltransportation['VehicleDepot'].Pos.y, Config.illegaltransportation['VehicleDepot'].Pos.z, true) < 1.5) then
								sleep = 5
								DrawText3Ds(Config.illegaltransportation['VehicleDepot'].Pos.x, Config.illegaltransportation['VehicleDepot'].Pos.y, Config.illegaltransportation['VehicleDepot'].Pos.z+1.0, 'Pour retirer votre ta paie, appuyez sur [~g~G~w~]')
								...
								if IsControlJustReleased(0, Keys['G']) and IsPedInAnyVehicle(ped, false) then
									...
									ESX.TriggerServerCallback('inside-illegaltransportation:payout', function(money)
										exports.pNotify:SendNotification({text = '<b>Transport illégal</b></br>vous avez gagné '..money..'$', timeout = 1500})
									end)
									Place = nil
			...
			elseif JobDone then
				exports.pNotify:SendNotification({text = "<b>Illegal Transportation</b></br>Aller à l'endroit suivant!", timeout = 4000})
				JobDone = false
				Place = Randomize(Config.Randomize)
				PlaceB()
				DrugObject = CreateObject(GetHashKey('hei_prop_heist_weed_pallet'), Config.Places[Place.name].DrugPos.x, Config.Places[Place.name].DrugPos.y, Config.Places[Place.name].DrugPos.z-1, false, true, true)
