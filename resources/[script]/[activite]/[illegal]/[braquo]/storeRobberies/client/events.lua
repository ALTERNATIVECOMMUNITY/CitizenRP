            ...
            for k, jobname in pairs(Config.CopJobs) do --Runs for all the job names set in the config.
                if(ESX.PlayerData.job.name == jobname)then
                  ...
                  --      TriggerEvent('chatMessage', 'DISPATCH', 'your-code-here', 'Store Robbery in progress at ' .. Config.Shops[store].name ..  ' (CCTV: ' .. CCTV .. ')')
                      ...
                      --  TriggerEvent('chatMessage', 'DISPATCH', 'your-code-here', 'PENIS' .. Config.Shops[store].name ..  '.')
                      --    local store = AddBlipForRadius(targetCoords.x, targetCoords.y, targetCoords.z, 50.0)

                        exports['core_dispach']:addMessage('Braquage de superette', {Config.Shops[v].shopKeeperCoords}, 'police', 5000, 11, 5)
                   exports['core_dispach']:addMessage('Braquage de superette',  {Config.Shops[v].shopKeeperCoords}, 'sheriff', 5000, 11, 5)
                    end
