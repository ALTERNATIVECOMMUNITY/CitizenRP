    ...
    --[[ MASS DEBUG END ]] --
        if GetPlayerName(user) == nil or GetPlayerName(user) ~= name then
            ...
            if connecting[license] ~= nil then
                ...
                if Config.Debug then
                    print(prefix .. " (Thread : NO LONGER EXISTS) currentConnectors was decremented... FROM: " ..
                              tostring(currentConnectors + 1) .. " -- TO: " .. tostring(currentConnectors));
                end
            ...
            if (GetPlayerCount() + 1) > slots or (GetPlayerCount() + Queue:GetMax()) > slots then
                ...
                if not Queue:IsSetUp(user) then
                    ...
                    if GetPlayerName(user) ~= nil then
                        sendToDiscQueue("QUEUED USER", "Player `" .. GetPlayerName(user):gsub("`", "") ..
                            "` has been added to the queue...", "Bad-DiscordQueue created by Badger");
                    end
                    local message = GetMessage(user);
                    local msg = message:gsub("{QUEUE_NUM}", Queue:GetQueueNum(user)):gsub("{QUEUE_MAX}", Queue:GetMax());
                    if GetPlayerName(user) ~= nil then
                        print(
                            prefix .. " " .. "Player " .. GetPlayerName(user) .. " has been set to the QUEUE [" .. msg ..
                                "]");
                    end
                end
                while (((not Queue:CheckQueue(user, currentConnectors, slots)) or (currentConnectors == maxConnectors)) or
                    (GetPlayerCount() == slots) or ((GetPlayerCount() + currentConnectors + 1) > slots) or
                    ((GetPlayerCount() + currentConnectors) >= slots)) do
                ...
                if connecting[license] == nil or connecting[license] ~= true then
                    ...
                    Wait(1000);
                    if GetPlayerName(user) ~= nil then
                        sendToDisc("NEW CONNECTOR",
                            "Player `" .. GetPlayerName(user):gsub("`", "") .. "` is allowed to join now!",
                            "Bad-DiscordQueue created by Badger");
                    end
...
AddEventHandler('playerDropped', function(reason)
    ...
    if (connecting[license] ~= nil) then
        if (currentConnectors > 0) then
            currentConnectors = currentConnectors - 1;
            if Config.Debug then
                print(prefix .. " (playerDropped) currentConnectors was decremented... FROM: " ..
                          tostring(currentConnectors + 1) .. " -- TO: " .. tostring(currentConnectors));
            end
        end
        connecting[license] = nil;
    ...
    if (Queue:IsSetUp(user)) then
        Queue:Pop(user);
        sendToDiscQueue("REMOVED QUEUE USER",
            "Player `" .. GetPlayerName(user):gsub("`", "") .. "` has been removed from the queue...",
            "Bad-DiscordQueue created by Badger");
...
AddEventHandler('DiscordQueue:Activated', function()
    ...
    if (currentConnectors > 0) then
        currentConnectors = currentConnectors - 1;
        if Config.Debug then
            print(prefix .. " (DiscordQueue:Activated) currentConnectors was decremented... FROM: " ..
                      tostring(currentConnectors + 1) .. " -- TO: " .. tostring(currentConnectors));
        end
    end
    if Config.Debug then
        print(prefix .. " (DiscordQueue:Activated) " .. "Player " .. GetPlayerName(user) ..
